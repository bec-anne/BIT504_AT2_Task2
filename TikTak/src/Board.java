...
public class Board {
	    ...
	    for (int row = 0; row < GameMain.ROWS; ++row) {
	        ...
	        for (int col = 0; col < GameMain.COLS; ++col) {
	            ...
	            if (cells[row][col].content == Player.Empty) {
	                // If there's an empty cell, it can't be a draw, so return false
	                return false;
	            }
	        }
	    }
	    // If no cells are empty, then it's a draw, so return true
	    return true;
	}
	...
	public boolean hasWon(Player thePlayer, int playerRow, int playerCol) {
		...
		if (playerRow + playerCol == 2 &&
		        ...
		        return true; // Player has won by completing the anti-diagonal
		    }
